...
XBASH_CHAR_GROUP_RE='([^]['$'\t'' !"#$%'\''(),./:;?@\^`{}~&*+<=>|-][^]['$'\t'' !"#$%'\''(),./:;?@\^`{}~&*+<=>|]*[./]?|[][!"#$%'\''(),./:;?@\^`{}~]|[&*+<=>|-]+)'
XBASH_SNIPPETS_COMMENTS_FIRST=1
...
xbash_includedirs() {
	local _lc_all_old _lc_all_old_isset=0 _file _has_errors=0
	while [ $# -ne 0 ]; do
		[ -d "$1" ] && {
			[ -n "${LC_ALL+x}" ] && {
				_lc_all_old_isset=1
			...
			LC_ALL=C
			for _file in "$1"/*; do
				if [ $_lc_all_old_isset -eq 1 ]; then
					LC_ALL=$_lc_all_old
				else
					unset -v LC_ALL
				fi
...
xbash_set_default_menu() {
	...
	fzf)
		...
		XBASH_MENU_COMMAND='\fzf --no-mouse --cycle --no-hscroll --height="$XBASH_MENU_HEIGHT" --reverse --tabstop=4 --color=16 --bind="$XBASH_MENU_CONF_FZF_BINDS"'
...
# Outputs a snippets to standard output.
xbash_compgen_snippets() {
	{
		GLOBIGNORE=.:..; LC_ALL=C; shopt -s nullglob
		printf '//%s\n' \
			~/.xbash/snippets/* \
			/etc/xbash/snippets/* \
			/usr/local/share/xbash/snippets/* \
			/usr/share/xbash/snippets/*
	} | xbash_awk '
	function print_str() {
...
xbash_compcontext_before_command() {
	...
	COMP_VALUES=("$(
		xbash_compgen_before_command | xbash_awk 'BEGIN {
			ARGV[1] = tolower(ARGV[1])
			comments_first = ARGV[2] + 0
		} {
			if(!xbash_startswith(tolower($0), ARGV[1])) next
			if(comments_first && sub(/\33$/, "")) {
				comment = ""
				if(match($0, /[\t ]+#[\t ]*/)) {
				...
				if(comment != "") print comment "  # " $0 "\33"
				else print $0 "\33"
			} else print $0
		}' "$COMP_VALUE" "$XBASH_SNIPPETS_COMMENTS_FIRST" | xbash_menu
	)") ||
	case $? in
	2)
		COMP_VALUES=("$(
	...
	*) return;;
	...
	if [ "${COMP_VALUES[0]: -1}" = $'\e' ]; then
		COMP_LINE_REPLACE=$(xbash_awk 'BEGIN {
			comments_first = ARGV[2] + 0
			if(comments_first || !sub(/[\t ]+#.*/, "", ARGV[1]))
				ARGV[1] = substr(ARGV[1], 1, length(ARGV[1]) - 1)
			if(comments_first)
				sub(/.*#[\t ]+/, "", ARGV[1])
			print xbash_ol2ml(ARGV[1])
		}' "${COMP_VALUES[0]}" "$XBASH_SNIPPETS_COMMENTS_FIRST")
